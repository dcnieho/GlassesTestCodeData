...
def preprocessData(inputDir, output_root, pid):
    """
    Run all preprocessing steps for EyeRecToo data
    """

    ## create the output directory (if necessary)
    outputDir = join(output_root, pid)
    ...
    with open(csv_file, 'w', encoding='utf-8', newline='') as csvfile:
        ...
        for g in list(chain(*gazeData_world[export_range])):
            data = ['{:.3f}\t{:d}\t{:.2f}\t{:.3f}\t{:.3f}'.format(g["timestamp"],
                g["frame_idx"],
                g["confidence"],
                g["norm_pos"][0],
                g["norm_pos"][1])]
    ...
    if not 'worldCamera.mp4' in os.listdir(outputDir):
        ...
        cmd_str = ' '.join(['ffmpeg', '-i', join(inputDir, 'Field.mp4'), '-i', join(inputDir, 'audio.wav'), '-pix_fmt', 'yuv420p', '-acodec', 'aac', join(inputDir, 'worldCamera.mp4')])
        os.system(cmd_str)
...
def getConfidence(entry):
    if int(entry['field.gaze.valid']) == 0:
        return 0
    lc = float(entry['left.pupil.confidence'])
    rc = float(entry['right.pupil.confidence'])
    if lc >= 0.66 and rc >= 0.66:
        return 0.5 * (lc + rc)
    else:
        return max(lc, rc)

def getNormedGaze(entry):
    gx = 0
    gy = 0
    width = float(entry['field.width'])
    height = float(entry['field.height'])
    if width > 0 and height > 0:
        gx = float(entry['field.gaze.x']) / width
        gy = float(entry['field.gaze.y']) / height
    return (gx, gy)

def formatGazeData(inputDir):
    """
    - load the journal data and timestamps
    - get the "gaze" fields from journal data (i.e. the gaze location w.r.t. the field camera)
    - sync gaze data with the field timestamps array
    """

    # load journal data
    journal_path = join(inputDir, 'post/JournalData.tsv')
    gaze_list = []
    with open(journal_path, 'r') as f:
        reader = DictReader(f, delimiter='\t')
        for entry in reader:
            if entry['field.timestamp'] == '9223372036854775807':
                continue
            entry['timestamp'] = float(entry['sync.timestamp'])
            entry['confidence'] = getConfidence(entry)
            entry['norm_pos'] = getNormedGaze(entry)
            gaze_list.append(entry)

    # load timestamps
    timestamps_path = join(inputDir, 'FieldData.tsv')
    frame_timestamps = []
    ...
    frame_timestamps = (frame_timestamps - start_timeStamp)
...
if __name__ == '__main__':
    ...
    parser.add_argument('inputDir', help='path to the raw EyeRecToo recording dir')
