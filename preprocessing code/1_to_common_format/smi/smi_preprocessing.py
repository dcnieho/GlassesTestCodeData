...
def preprocessData(inputDir, output_root, pid):
    ...
    newDataDir = copySMI_recording(inputDir, output_root, pid)
    ...
    cmd_str = ' '.join(['ffmpeg', '-i', join(newDataDir, 'SMI_worldCamera.mp4'), '-i', join(newDataDir, 'audio.wav'), '-pix_fmt', 'yuv420p', '-acodec', 'aac', join(newDataDir, 'worldCamera.mp4')])
    os.system(cmd_str)
...
def copySMI_recording(inputDir, output_root, pid):
    """
    ...
    for f in os.listdir(inputDir):
        # movie file
        if f.endswith('-recording.avi'):
            shutil.copy(join(inputDir, f), join(outputDir, 'SMI_worldCamera.avi'))

        # wav file
        if f.endswith('-recording.wav'):
            shutil.copy(join(inputDir, f), join(outputDir, 'audio.wav'))
...
def formatGazeData(input_dir):
    ...
    raw_df = pd.read_table(join(input_dir, 'SMI_raw.txt'), skiprows=32)
    ...
    #        conf[i] = 1
    conf = np.zeros(shape=raw_df.shape[0])
    ldiam = raw_df['L Dia X [px]']
    rdiam = raw_df['R Dia X [px]']
    for i in range(0, len(ldiam)):
        if float(ldiam[i]) > 0:
            conf[i] += 0.5
        if float(rdiam[i]) > 0:
            conf[i] += 0.5
